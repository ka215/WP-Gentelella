...
if ( ! in_array( $profileuser->display_name, $public_display ) ) // Only add this if it isn't duplicated elsewhere (:> 別の場所に複製されていない場合にのみ追加します
...
foreach ( $public_display as $id => $item ) : ?>
          <option <?php selected( $profileuser->display_name, $item ); ?>><?= esc_html( $item ); ?></option>
<?php endforeach; ?>
        </select>
      </div>
    </div>
...
<?php endforeach; ?>
    <div class="form-group">
      ...
      <div class="col-sm-10">
        ...
        <span class="help-block"><?php _e( 'Share a little biographical information to fill out your profile. This may be shown publicly.', WPGENT_DOMAIN ); ?></span>
      </div>
    </div>
    <div class="clearfix"></div>
<?php do_action( 'show_user_profile', $profileuser ); ?>
    ...
    <div class="form-group">
      <div class="col-sm-offset-2 col-sm-10">
        ...
        <button type="submit" name="submit" id="submit" class="btn btn-primary"><?php esc_attr_e( 'Update Profile', WPGENT_DOMAIN ); ?></button>
      </div>
    </div>
  </form>
</div>
